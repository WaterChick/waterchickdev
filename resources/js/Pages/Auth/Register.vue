<template>
  ...
  <div class="min-h-screen flex items-center justify-center p-4 relative overflow-hidden select-none">
    <div class="relative w-full max-w-md">
      <div class="bg-white/10 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 p-8 relative overflow-hidden">
        ...
        <div class="relative z-10">
          <div class="text-center mb-8">
            <h1 class="text-4xl font-bold text-white mb-2">{{ $t('Register') }}</h1>
            <p class="text-[#60A5FA]/80">{{ $t('Enter your details below to create your account') }}</p>
          </div>

          <form @submit.prevent="submit" class="space-y-6">
            <FormControl>
              <Label for="email" class="text-white/90 font-medium mb-2 block">{{ $t('Name') }}</Label>
              <div class="relative">
                <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                  <svg class="w-5 h-5" style="color: #60A5FA" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 20v-2a6 6 0 0112 0v2H6z" />
                </svg>
                </div>
                <Input 
                    id="name"
              ...
              <p v-if="form.errors.name" class="mt-2 text-sm" style="color: #60A5FA">{{ $t(form.errors.name) }}</p>
            </FormControl>
      ...
      <div class="absolute -bottom-4 -right-4 w-24 h-24 rounded-full blur-2xl opacity-50" style="background: linear-gradient(135deg, #60A5FA 0%, #60A5FA 100%)"></div>
      <div class="absolute -top-4 -left-4 w-24 h-24 rounded-full blur-2xl opacity-50" style="background: linear-gradient(135deg, #60A5FA 0%, #60A5FA 100%)"></div>
    </div>
  </div>
</template>
