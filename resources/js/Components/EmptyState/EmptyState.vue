<template>
  <div :class="cn('overflow-hidden flex items-center flex-col', $attrs.class || '')">
    <div class="flex flex-col max-w-lg items-center relative">
      ...
      <p class="font-semibold text-center">{{ $t(title) }}</p>
      <p class="max-w-md text-center text-muted-foreground text-sm mt-1">{{ $t(message) }}</p>
    </div>
  </div>
</template>
